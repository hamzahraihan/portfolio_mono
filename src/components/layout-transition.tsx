...
export function LayoutTransition({ children, className, style, initial, animate, exit }: Readonly<LayoutTransitionProps>) {
  const segment = useSelectedLayoutSegment();

  return (
    <AnimatePresence mode="wait" initial={false}>
      <motion.div className={className} transition={{ ease: 'easeInOut', duration: 0.75 }} style={style} key={segment} initial={initial} animate={animate} exit={exit}>
